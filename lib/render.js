...
function calculateMetatileKartenbauer(options, config) {
	...
	var maxRes = config.maxScale;
	//meters per pixel to draw for this scale
    var resolution = maxRes / total;
	...
	/* the bounds are [west, north, east, south]
    ...
    var west = (x * 256 * resolution * xdir) + b[0];
    var north = ((y + metaHeight) * 256 * resolution * ydir) + b[1];
    var east = ((x + metaWidth) * 256 * resolution * xdir) + b[0];
    var south = (y * 256 * resolution * ydir) + b[1];
    return {
        width: metaWidth * options.tileSize,
        height: metaHeight * options.tileSize,
        x: x, y: y,
...
MapnikSource.prototype._renderMetatile = function(options, callback) {
    ...
    if(source._uri.kartenbauer) {
		meta = calculateMetatileKartenbauer(options, source._uri.kartenbauer);
	} else {
    ...
    process.nextTick(function() {
        source._pool.acquire(function(err, map) {
			...
			if(source._uri.kartenbauer && source._uri.kartenbauer.lockToDesignScale) {
				console.log(map.scale());
				console.log(source._uri.kartenbauer.lockToDesignScale);
				options.scale = source._uri.kartenbauer.lockToDesignScale / map.scale();
			}
