...
MapnikSource.prototype._createPool = function(xml, callback) {
    ...
    if (!this._pool) this._pool = Pool({
        ...
        max: Math.ceil(require('os').cpus().length * 1.5)
    });
    callback(null);
};
...
MapnikSource.prototype.getTile = function(z, x, y, callback, id_callback) {
    z = +z; x = +x; y = +y;
    ...
    var cb_id = this._tileCache.get(key, function(err, tile, headers) {
        if (err) return callback(err);
...
MapnikSource.prototype.cancelRequest = function(cb_id) {
	this._tileCache.removeCallback(cb_id);
};
